...
# --- Translation Dictionary (Keep as is) ---
translations = {
    'en': {
    ...
    'pt': {
        ...
        'legend_location': 'center left', #'best', # Example: change legend location for pt
        'legend_anchor': (1, 0.5), # Keep anchor same unless needed
...
logger.info(get_translations('setting_up_logging', 'en')) # Log setup message early
...
if output_dir_created:
    for lang in LANGUAGES:
        ...
        try:
            ...
            # logger.info(get_translation('log_file_created', lang).format(log_file_path)) # Log this later
...
def process_csv(filepath, label, lang='en'):
    ...
    try:
        ...
        logger.error(f"Error: File not found: {filepath}") # Keep critical errors clear
        return None, None, None
...
for label, filename in BASELINE_FILES.items():
    ...
    if not os.path.exists(filepath):
        ...
        df, processed_label, _ = process_csv(filepath, label, lang)
        if df is not None:
...
if missing_baseline:
    ...
    for handler in current_lang_handlers.values():
...
for filepath in test_files:
    ...
    if speed_val is not None:
        ...
        df, processed_label, speed_out = process_csv(filepath, label, lang)
...
plot_order = list(BASELINE_FILES.keys()) + [f"{speed} RPM" for speed in sorted_speeds] # Define plot order
...
if len(rpm_labels_for_corr) >= 2:
    rpms = [int(re.search(r'(\d+)', label).group(1)) for label in rpm_labels_for_corr]
    cpu_temps_at_rpm = [steady_state_means[label]['CPU'] for label in rpm_labels_for_corr]
    gpu_temps_at_rpm = [steady_state_means[label]['GPU'] for label in rpm_labels_for_corr]
    try:
...
for lang in LANGUAGES:
    ...
    if len(plot_order_steady) >= 2 : # Need at least two for comparison
        try:
            cpu_data_for_boxplot = []
            gpu_data_for_boxplot = []
            labels_for_boxplot = [] # Use the steady state labels

            for label in plot_order_steady: # Use filtered list
                df = all_data[label]
                steady_df = df[df['Elapsed_Time'] >= STEADY_STATE_START_TIME]
                # We already checked for valid means, steady_df should not be empty here
                cpu_data = steady_df['CPU_Avg_Temp'].dropna()
                gpu_data = steady_df['GPU_Temp'].dropna()
                if not cpu_data.empty and not gpu_data.empty: # Ensure there's data after dropna
                    cpu_data_for_boxplot.append(cpu_data)
                    gpu_data_for_boxplot.append(gpu_data)
                    labels_for_boxplot.append(label)
                else:
                     logger.warning(f"Skipping label '{label}' for boxplot ({lang}) due to empty data after dropna/steady state.")

            # Check again if we still have enough data after potential drops
    ...
    if steady_state_means and len(plot_order_steady) > 0:
        try:
            ...
            rects2 = ax.bar(x + width/2, gpu_means, width, label=get_translation('plot_bar_legend_gpu', lang), color=[color_map_steady.get(l,'lightgrey') for l in plot_order_steady], alpha=0.6)
            ...
            try:
                ...
                ax.bar_label(rects2, padding=3, fmt='%.1f')
                 ...
                 logger.warning("ax.bar_label not available (requires Matplotlib >= 3.4). Skipping bar labels.")
    ...
    if len(rpm_labels_for_corr) >= 2: # Use the list created before loop
        try:
            # Data already extracted before loop (rpms, cpu_temps_at_rpm, gpu_temps_at_rpm)
            ...
            try:
                if len(rpms) >= 2: # Need at least 2 points for polyfit
                    ...
                    if rpm_min == rpm_max: # Handle case with only two points at same RPM (unlikely but safe)
                ...
                if not np.isnan(corr_cpu):
            ...
            except (np.linalg.LinAlgError, ValueError, TypeError) as fit_err: # Added TypeError
                 ...
                 # Original correlation error message already logged during calculation
            ...
            try:
                ...
                # Original correlation error message already logged
    ...
    # Remove the handler specific to this language iteration if it was added
    if added_handler and lang != 'en': # Don't remove 'en' handler yet
        ...
        del current_lang_handlers[lang] # Remove from active dict

# --- Final Correlation Summary (Logged once to EN file and console) ---
if len(rpm_labels_for_corr) >= 2:
    logger.info(get_translation('correlation_analysis_title', 'en'))
    logger.info(get_translation('correlation_cpu_label', 'en').format(corr_cpu if not np.isnan(corr_cpu) else float('nan'))) # Handle NaN for printing
    logger.info(get_translation('correlation_gpu_label', 'en').format(corr_gpu if not np.isnan(corr_gpu) else float('nan')))
    ...
    # else: Cannot evaluate if corr is NaN
...
# else: Skip message already logged inside the loop

logger.info(get_translation('processing_complete', 'en'))

# --- Final Cleanup ---
# Remove and close any remaining file handlers (should just be 'en' at this point)
for lang_code, handler in current_lang_handlers.items():
